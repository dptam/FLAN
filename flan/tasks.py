...
FLAN_OUTPUT_FEATURES = {
    "inputs": seqio.Feature(vocabulary=FLAN_VOCABULARY, add_eos=True, required=False),
...
def _process_cnn_dailymail(example):
    ...
    for sep in ["2013 . ", "2014 . ", "2012 . ", ") -- "]:
        article = tf.strings.split(article, sep=sep, result_type="RaggedTensor")[-1][0]
    return {
        "text": article,
...
TASK_CONFIGS["cnn_dailymail"] = _TaskConfig(
    source=seqio.TfdsDataSource(tfds_name="cnn_dailymail:3.1.0", splits=SPLITS_DICT),
    preprocessors=[
...
def _process_opinion_abstracts_rotten_tomatoes(example):
    return {
        ...
        "numbered_reviews": enumerate_items(example["_critics"]["value"][:10]),
    }
...
def _process_opinion_abstracts_idebate(example):
    return {
        ...
        "argument_sentences": enumerate_items(
...
TASK_CONFIGS["paws_wiki"] = _TaskConfig(
    source=seqio.TfdsDataSource(tfds_name="paws_wiki:1.1.0", splits=SPLITS_DICT),
    preprocessors=[
        _process_paws_wiki,
        preprocessors.format_options,
    ],
...
def register_few_shot_versions_of_task(
    zero_shot_name: str,
...
def register_few_shot_versions_of_continuations_task(zero_shot_name: str) -> None:
...
for t_name, config in TASK_CONFIGS.items():
    ...
    for idx, patterns in enumerate(templates.PATTERNS[flan_pattern_name]):
        ...
        if utils.is_classification(flan_pattern_name):
            zero_shot_task_name = utils.ZeroshotScoreEvalTaskName.get(t_name, idx)
        ...
        if utils.is_classification(flan_pattern_name):
            ...
            seqio.TaskRegistry.add(
                ...
                # Format inputs and outputs according to the patterns. This should be
                # the same for all tasks.
                preprocessors.get_flan_formatter(
                    inputs_pattern_no_options, targets_pattern
                )
                + [preprocessors.rank_classification_from_options]
                +
                # Tokenization for the prefix-LM. This should be the same for all
    ...
    for num_templates in NUM_TEMPLATES_LIST:
        ...
        zero_shot_task_name = utils.ZeroshotTemplatedTaskName.get(t_name, num_templates)
        seqio.TaskRegistry.add(
            zero_shot_task_name,
            source=config.source,
            preprocessors=config.preprocessors +
