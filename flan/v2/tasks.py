...
_niv2_exemplar_inputs = [
    _flatten([y["input"] for y in x["sample"]]) for x in _niv2_few_shot_exemplars
]
_niv2_exemplar_targets = [
    _flatten([y["output"] for y in x["sample"]]) for x in _niv2_few_shot_exemplars
]
...
def register_zero_shot_task(
    ...
    add_template_metadata_fn = functools.partial(
        prep.add_template_info, template_type=template_type
    )
...
def register_niv2_few_shot_task(
    ...
    for suffix, output_features in constants.TRAIN_TASK_SUFFIXES_AND_FEATURES:
        seqio.TaskRegistry.add(
            ...
            + prep.get_batch_formatter(patterns)
            + prep.FLAN_TOKENIZE,
            postprocess_fn=zero_shot_config.postprocess_fn,
            output_features=output_features,
            metric_fns=zero_shot_config.metric_fns,
...
for t_name, config in task_configs.ALL_CANDIDATE_TASK_CONFIGS.items():
    ...
    for pattern_idx in range(10):
        ...
        register_zero_shot_task(
            zero_shot_task_name, config, selected_patterns, "zs_opt"
        )
    ...
    register_zero_shot_task(
        zero_shot_task_name, config, no_opt_selected_patterns, "zs_noopt"
    )
    ...
    if prep.format_options in config.preprocessors:
        ...
        register_zero_shot_task(
            zero_shot_task_name, config_non_deter_opt, selected_patterns, "zs_opt"
        )
    ...
    if prep.format_dialog in config.preprocessors:
        ...
        zero_shot_task_name = f"{t_name}_template_0to10_non_deter_opt_zero_shot"
        register_zero_shot_task(
            zero_shot_task_name, config_non_deter_dialog, selected_patterns, "zs_opt"
        )
...
for t_name, config in NON_NIV2_TASK_CONFIGS.items():
    ...
    for pattern_dict, pattern_name in [
        (templates.FEWSHOT_PATTERNS, ""),
        (templates.FEWSHOT_PATTERNS_NO_OPTIONS, "_no_opt"),
    ]:
        ...
        for pattern_i, num_shots in [
            ...
            for (
                ...
                few_shot.register_few_shot_version_of_task(
                    ...
                    new_task_name=(
                        fewshot_base_task_name + num_shots.name_suffix + task_suffix
                    ),
...
for t_name, config in task_configs.NIV2_TASK_CONFIGS.items():
    ...
    register_zero_shot_task(
        f"{t_name}_template_mix_five_shot", config, mixed_templates, "fs_opt"
    )
    register_zero_shot_task(
        f"{t_name}_template_mix_no_opt_five_shot", config, mixed_templates, "fs_noopt"
    )

    few_shot_patterns = []
    for few_shot_pattern in x_shot_templates:
        few_shot_patterns.append(
            (
                few_shot_pattern.combined_inputs_w_target_prefix,
                few_shot_pattern.combined_targets_wo_target_prefix,
    ...
    for opt_type_name, template_type in [
        ...
        register_niv2_few_shot_task(
            few_shot_task_name, config, few_shot_patterns, template_type
        )
