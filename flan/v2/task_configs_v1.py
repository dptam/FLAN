...
def rouge_fn(targets: List[List[str]], predictions: List[str]) -> Dict[str, float]:
    ...
    for targ_for_resp, resp in tqdm.tqdm(zip(targets, predictions), total=len(targets)):
        ...
        for rouge_type in rouge_types:
            ...
            best_score_index = max(range(len(resp_scores)), key=_f)
    ...
    results = {}
    for rouge_type in rouge_types:
        results[rouge_type] = t5_metrics.rouge(max_references[rouge_type], predictions)[
            rouge_type
        ]
    return results
...
def _process_wsc(example):
    ...
    options = tf.constant(["no", "yes"])
    return {
        "context": tf.strings.regex_replace(
...
wsc_source_args = {"tfds_name": "super_glue/wsc.fixed:1.0.2", "splits": WSC_SPLITS_DICT}
"""
...
def _process_wsc273(example):
    ...
    entities = tf.stack([example["option1_normalized"], example["option2_normalized"]])
...
def _process_record(example):
    ...
    query_left = tf.strings.strip(
        tf.strings.split(example["query"], "@placeholder", result_type="RaggedTensor")[
            0
        ][0]
    )
    ...
    query_right = tf.strings.split(
        example["query"], "@placeholder", result_type="RaggedTensor"
    )[0][1]
    ...
    passage = tf.strings.split(
        example["passage"], "\n@highlight", result_type="RaggedTensor"
    )[0][0]
...
def _process_cnn_dailymail(example):
    ...
    for sep in ["2013 . ", "2014 . ", "2012 . ", ") -- "]:
        article = tf.strings.split(article, sep=sep, result_type="RaggedTensor")[0][-1]
...
TASK_CONFIGS["cnn_dailymail"] = TaskConfig(
    source=seqio.TfdsDataSource(tfds_name="cnn_dailymail:3.4.0", splits=SPLITS_DICT),
    preprocessors=[
...
TASK_CONFIGS["multi_news"] = TaskConfig(
    source=seqio.TfdsDataSource(tfds_name="multi_news:1.0.0", splits=SPLITS_DICT),
    preprocessors=[
        _filter_multi_news,
        _process_multi_news,
    ],
...
def _process_opinion_abstracts_rotten_tomatoes(example):
    return {
        "critic_consensus": example["_critic_consensus"],
        "movie": tf.regex_replace(  # Should be title case...
            example["_movie_name"], "_", " "
        ),
        "first_review": example["_critics"]["value"][0],
        "numbered_reviews": prep.numbered_items_str(example["_critics"]["value"][:10]),
    }


oart_prep_fn = functools.partial(
    prep.add_source_info,
    task_name="opinion_abstracts_rotten_tomatoes",
...
def _process_opinion_abstracts_idebate(example):
    return {
        "claim": example["_claim"],
        "debate_name": example["_debate_name"],
        "argument_sentences": prep.numbered_items_str(
            example["_argument_sentences"]["value"][:10]
        ),
    }


oaid_prep_fn = functools.partial(
    prep.add_source_info, task_name="opinion_abstracts_idebate", task_source="Flan2021"
)
"""
...
def _process_coqa(example):
    return {
        "text": example["story"],
        "numbered_questions": prep.numbered_items_str(example["questions"]),
        "numbered_answers": prep.numbered_items_str(example["answers"]["input_text"]),
    }
...
def _process_quac(example):
    def _remove_cannotanswer(str_field):
        return tf.strings.strip(tf.strings.regex_replace(str_field, "CANNOTANSWER", ""))
...
def _process_multirc(example):
    ...
    glm_options = tf.strings.join([glm_options_left, glm_options_right], separator=" ")
    return {
...
def flan_post_multirc(string_label, example=None, is_target=False):
    ...
    res = {
        "value": t5_post.string_label_to_class_id(
...
def _process_sentiment140(example):
    ...
    is_positive = tf.logical_or(
        tf.math.equal(example["polarity"], 3), tf.math.equal(example["polarity"], 4)
    )
...
def _process_imdb_reviews(example):
    ...
    one_hot = tf.one_hot(label, 2)
    options = tf.constant(["negative", "positive"])
    glm_options = tf.constant(["negative movie review.", "positive movie review."])
    return {
...
def _process_winogrande(example):
    ...
    label = tf.cast(example["label"], tf.int32)
    one_hot = tf.one_hot(label, 2)
    context = tf.strings.split(example["sentence"], "_", result_type="RaggedTensor")[0][
        0
    ]
    next_sentence = tf.strings.split(
        example["sentence"], "_", result_type="RaggedTensor"
    )[0][1]
    options = tf.stack(
        [
...
pc_prep_fn = functools.partial(
    prep.add_source_info, task_name="para_crawl_enes", task_source="Flan2021"
)
...
def _process_common_gen(example):
    references = tf.concat(
        [example["references"], tf.expand_dims(example["target"], axis=0)], axis=0
    )
    return {
        "concepts": tf.strings.reduce_join(example["concepts"], separator=", "),
        "concepts_newline": tf.strings.reduce_join(example["concepts"], separator="\n"),
...
def _process_dart(example):
    ...
    tripleset_newline = tf.strings.reduce_join(example["tripleset"], separator="\n")
...
def _process_e2e_nlg(example):
    ...
    meaning_representation = tf.regex_replace(
        example["meaning_representation"], r"\[", " = "
    )
...
tc_prep_fn = functools.partial(
    prep.add_source_info, task_name="true_case", task_source="Flan2021"
)
...
fp_prep_fn = functools.partial(
    prep.add_source_info, task_name="fix_punct", task_source="Flan2021"
)
...
ws_prep_fn = functools.partial(
    prep.add_source_info, task_name="word_segment", task_source="Flan2021"
)
...
def _process_qnli(example):
    ...
    glm_options_right = tf.fill(tf.shape(glm_options_left), value=example["sentence"])
    glm_options = tf.strings.join([glm_options_left, glm_options_right], separator=" ")
    return {
        "sentence": example["sentence"],
        "question": example["question"],
...
def _process_openbookqa(example):
    ...
    options = [
        example["question"]["choice_A"],
        example["question"]["choice_B"],
        example["question"]["choice_C"],
        example["question"]["choice_D"],
    ]
    return {
